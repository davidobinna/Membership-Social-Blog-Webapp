           ...
           {{-- Free Plan --}}
            ...
            <div class="border border-gray-200 divide-y divide-gray-200 shadow-lg bg-gray-50">
                <div class="p-6">
                    ...
                    <h2>
                        {{ dump(auth()->user()->subscribedToProduct($plan->stripeProductId(), $plan->stripeName())) }}
                    </h2>

                    @subscribedToProduct(auth()->user(), $plan->stripeProductId(), $plan->stripeName())
                    <h2 class="p-2 mt-4 text-white rounded bg-theme-blue-200">You are currently subscribed to a plan</h2>
                    @else
                    <x-link.primary class="mt-6" href="{{ route('payments',['plan' => $plan->stripeName()]) }}">
                        Sign Up
                    </x-link.primary>
